...
import { Check, ChevronDown, ChevronUp, Eye, Palette } from 'lucide-react';
    ...
    if (isAdmin) {
      ...
      const success = await saveGlobalThemeConfig({
        ...
        allowUserCustomization:
          globalThemeConfig?.allowUserCustomization ?? true,
      });
          ...
          <div className='space-y-4'>
            <div className='p-3 bg-theme-accent/5 border border-theme-accent/20 rounded-lg'>
              ...
              <div className='text-xs text-theme-text-secondary mt-1'>
                默认主题:{' '}
                {themes.find((t) => t.id === globalThemeConfig.defaultTheme)
                  ?.name || globalThemeConfig.defaultTheme}
                {globalThemeConfig.customCSS && ' | 包含自定义CSS'}
                {!globalThemeConfig.allowUserCustomization &&
                  ' | 禁止用户自定义'}
              </div>
            </div>
          ...
          {themes.map((theme) => (
            <div
              ...
              className={`relative p-4 border-2 rounded-xl transition-all ${
                ...
                isAdmin
                  ? 'cursor-pointer hover:border-theme-accent/50'
                  ...
                  <button
                    ...
                    className={`p-1 transition-colors ${
                      isAdmin
                        ...
                        : 'text-theme-text-secondary opacity-50 cursor-not-allowed'
                    }`}
          ...
          <div className='space-y-4'>
            <div className='text-sm text-theme-text-secondary bg-theme-surface p-3 rounded-lg border border-theme-border'>
              ...
              <ul className='space-y-1 text-xs'>
                <li>
                  • 使用CSS变量覆盖主题颜色：
                  <code className='bg-theme-bg px-1 rounded'>
                    --color-theme-accent: 255, 0, 0;
                  </code>
                </li>
                <li>
                  • 使用Tailwind类名：
                  <code className='bg-theme-bg px-1 rounded'>{`.my-class { @apply bg-red-500; }`}</code>
                </li>
                <li>
                  • 自定义组件样式：
                  <code className='bg-theme-bg px-1 rounded'>{`.admin-panel { border-radius: 20px; }`}</code>
                </li>
                <li>• 修改会实时生效，请谨慎使用</li>
              </ul>
            </div>
  ...
  @apply bg-gradient-to-r from-purple-500 to-pink-500 text-white px-6 py-3 rounded-xl;
        ...
        <div className='bg-theme-surface border border-theme-border rounded-lg p-4'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3'>
            {cssTemplates.map((template) => (
              <div
                key={template.id}
                className='p-3 border border-theme-border rounded-lg hover:bg-theme-accent/5 transition-colors group'
              >
                <div className='flex items-center justify-between mb-2'>
                  <h5 className='text-sm font-medium text-theme-text'>
                    {template.name}
                  </h5>
                  <button
                    onClick={() =>
                      handleApplyTemplate(template.css, template.name)
                    }
                ...
                <div className='text-xs bg-theme-bg rounded p-2 max-h-16 overflow-y-auto'>
                  <code className='whitespace-pre-wrap text-theme-text-secondary'>
                    {template.preview}
                  </code>
                </div>
              </div>
          ...
          <div className='mt-4 p-3 bg-theme-accent/5 border border-theme-accent/20 rounded-lg'>
            <p className='text-xs text-theme-text-secondary'>
              <strong>💡 使用提示：</strong>{' '}
              点击模板的"应用"按钮将代码复制到自定义CSS编辑器，然后可以在此基础上进行修改。记得点击"应用样式"按钮生效。
            </p>
          </div>
        </div>
      )}
      ...
      <div className='bg-theme-surface border border-theme-border rounded-lg p-4'>
        ...
        <div className='text-sm text-theme-text-secondary space-y-2'>
          ...
          {isAdmin && (
            <p>
              <strong>自定义CSS：</strong>
              通过CSS变量或直接样式实现全站个性化定制
            </p>
          )}
          {isAdmin && (
            <p>
              <strong>样式模板：</strong>使用预设模板快速实现炫酷效果
            </p>
          )}
          <p>
            <strong>主题变量：</strong>
          ...
          <ul className='text-xs space-y-1 ml-4 mt-1'>
            <li>
              •{' '}
              <code className='bg-theme-bg px-1 rounded'>--color-theme-bg</code>{' '}
              - 背景色
            </li>
            <li>
              •{' '}
          ...
          {isAdmin && (
            <>
              ...
              <ul className='text-xs space-y-1 ml-4 mt-1'>
                <li>
                  • 修改背景：
                  <code className='bg-theme-bg px-1 rounded'>{`body { background: linear-gradient(...); }`}</code>
                </li>
                <li>
                  • 使用Tailwind：
                  <code className='bg-theme-bg px-1 rounded'>{`.my-class { @apply bg-red-500; }`}</code>
