...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'DranTV_play_records';
const FAVORITES_KEY = 'DranTV_favorites';
const SEARCH_HISTORY_KEY = 'DranTV_search_history';

// 缓存相关常量
const CACHE_PREFIX = 'DranTV_cache_';
...
class HybridCacheManager {
  ...
  private clearAllCache(): void {
    ...
    keys.forEach((key) => {
      if (key.startsWith('DranTV_cache_')) {
...
export async function getSkipConfig(
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('DranTV_skip_configs');
    if (!raw) return null;
...
export async function saveSkipConfig(
  ...
  try {
    ...
    configs[key] = config;
    localStorage.setItem('DranTV_skip_configs', JSON.stringify(configs));
    window.dispatchEvent(
      new CustomEvent('skipConfigsUpdated', {
        detail: configs,
      })
